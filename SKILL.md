---
name: typesafe-ai-opencode-free
license: MIT
description: >
...
## Local OpenCode free transport

This local variant preserves the TypeSafe guidance above. When issuing a real
System One request, call `bin/systemone-call` instead of constructing the
HTTP request directly. The helper uses OpenCode Zen's `jev-1.13-free` model
and handles its transport details; inspect its `--help` or source when you
need the invocation schema.
